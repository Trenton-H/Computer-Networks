...
void Router :: addRT(int d, int c, int nH)
{
    if(!(RT.at(dest).dest))
    {
        RT[dest].dest = d;
...
void Router :: periodicUpdate()
{
    ...
    for(int j = 0; j < 100; j++)
    {
        if(RT[j].cost > 0)
        {
            pack.packet[j].cost.push_back(RT[j].cost);
            pack.packet[j].dest.push_back(RT[j].dest);
        }
    }
    for(int i = 0; i < 100; i++)
    {
        if(neighbors.at(i).cost > 0)
            pack.dest = neighbors.at(i).neighbor;
        receivePacket(pack);
    }
}
...
void Router :: receivePacket(dvPacket p)
{
    for(int i = 0; i < 100; i++)
    {
        ...
        {
            RT.at(i).cost.push_back(p.packet[i].cost);
            RT.at(i).nextHop.push_back(p.src);
        }
    }
}
